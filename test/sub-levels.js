...
// sub levels
  ...
  logLevels.forEach(level => {
    ...
    t.same(JSON.parse(process.stdout.write.getCall(-1).args[0]),
      JSON.parse(`{"time":946684800000,"level":"${level}","message":"print ${level} with sub levels","hostname":"my-machine","hostip":"127.0.0.1","pid":123}`),
  ...
  subLevels.forEach(subLevel => {
    ...
    t.same(JSON.parse(process.stdout.write.getCall(-1).args[0]),
      JSON.parse(`{"time":946684800000,"level":"info","message":"print info: with sub level ${subLevel}","hostname":"my-machine","hostip":"127.0.0.1","pid":123,"subLevel": "${subLevel}"}`),
