...
// time options
  ...
  t.same(JSON.parse(process.stdout.write.getCall(-1).args[0]),
    JSON.parse('{"time":946684800000,"level":"info","message":"time format as epoch","hostname":"my-machine","hostip":"127.0.0.1","pid":123}'),
    'time format as epoch')

  const log2 = munia({timeFormat: 'iso'})
  log2.info('time format as iso')
  t.same(JSON.parse(process.stdout.write.getCall(-1).args[0]),
    JSON.parse('{"time":"2000-01-01T00:00:00.000Z","level":"info","message":"time format as iso","hostname":"my-machine","hostip":"127.0.0.1","pid":123}'),
    'time format as iso')

  const log3 = munia({timeFormat: 'local'})
  log3.info('time format as local')
  t.same(JSON.parse(process.stdout.write.getCall(-1).args[0]),
    JSON.parse('{"time":"2000-01-01T05:30:00.000+05:30","level":"info","message":"time format as local","hostname":"my-machine","hostip":"127.0.0.1","pid":123}'),
  ...
  t.same(JSON.parse(process.stdout.write.getCall(-1).args[0]),
    JSON.parse('{"time": "5:30:0","level":"info","message":"custom time format, time option as function","hostname":"my-machine","hostip":"127.0.0.1","pid":123}'),
